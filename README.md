...
**Zatochka** - это полнофункциональная CRM система для автоматизации бизнеса заточки и ремонта инструментов. Система включает в себя управление заказами, клиентами, филиалами, автоматические уведомления через Telegram и SMS, систему отзывов и бонусную программу.
...
-   **`app/Services/TelegramWebhookService.php`** - обработка Telegram webhook
-   **`app/Services/BonusService.php`** - бонусная система
-   **`app/Jobs/SendBirthdayGreeting.php`** - поздравления с ДР
-   **`app/Models/Order.php`** - модель заказа с полным workflow
-   **`app/Models/Client.php`** - модель клиента с Telegram интеграцией
-   **`resources/js/components/Reviews.vue`** - компонент отзывов
...
-   Защита от CSRF атак
-   Верификация Telegram аккаунтов

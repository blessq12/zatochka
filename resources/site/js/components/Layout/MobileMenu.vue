<script>
import SocialNetworkIcon from "../Contacts/SocialNetworkIcon.vue";
...
<template>
    <!-- Меню под sticky-навигацией (h-20), чтобы верхняя строка не прыгала -->
    <Teleport to="body">
    <transition name="mobile-menu">
        <div
            v-if="isOpen"
            class="lg:hidden fixed top-20 inset-x-0 bottom-0 bg-[#003859] z-[100] flex flex-col"
            role="dialog"
            aria-modal="true"
            aria-label="Мобильное меню"
        >
            <nav
                class="flex-1 flex flex-col items-center justify-center gap-4 sm:gap-5 px-6 overflow-y-auto py-6"
            >
                <a href="/" @click="closeMenu" :class="navLinkClass('/')">
                    ГЛАВНАЯ
                </a>
                <a
                    href="/sharpening"
                    @click="closeMenu"
                    :class="navLinkClass('/sharpening')"
                >
                    ЗАТОЧКА
                </a>
                <a
                ...
                <a
                    href="/work-schedule"
                    @click="closeMenu"
                    :class="navLinkClass('/work-schedule')"
                >
                    ГРАФИК РАБОТЫ
                </a>
                <a

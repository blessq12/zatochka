@php
    /** @var \App\Http\ViewModels\PublicSite\PublicSiteViewModel $site */
@endphp
<!-- Мобильное меню - полноэкранное -->
    <div
            data-mobile-menu
            class="lg:hidden hidden fixed inset-0 bg-[#003859] z-[100] flex-col"
        >
            <!-- Верхняя часть - Header -->
            <div
                class="flex justify-between items-center px-6 sm:px-8 lg:px-12 py-4 h-20"
            >
                <!-- Логотип -->
                <a href="{{ url('/') }}"
                    class="flex items-center space-x-2 sm:space-x-3 p-2 -m-2 flex-shrink-0{{ $site->isActive('/') ? ' bg-white/20' : '' }}"
                >
                    <svg
                        width="36"
                        height="25"
                        viewBox="0 0 36 25"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        class="w-12 h-8 flex-shrink-0"
                    >
                        <path
                            d="M25.3397 12.1789C25.3397 11.6708 24.9268 11.2578 24.4186 11.2578C23.9105 11.2578 23.4976 11.6708 23.4976 12.1789C23.4976 12.687 23.9105 13.1 24.4186 13.1C24.9268 13.1 25.3397 12.6889 25.3397 12.1789Z"
                            fill="#C20A6C"
                        />
                        <path
                            d="M32.9363 12.1868C32.9363 12.1868 32.9421 12.183 32.944 12.181C32.9421 12.1791 32.9382 12.1772 32.9363 12.1753C34.5178 11.0221 35.5988 9.27314 35.6558 6.92665C35.5493 2.4487 31.7241 0.147881 27.7847 0.0108596V0.00515036C27.7847 0.00515036 23.3239 -0.24225 20.5264 2.6923L23.1697 4.37272C25.0842 2.77032 27.9579 3.01963 27.9579 3.01963C29.0997 3.09004 32.0952 3.96926 32.2246 6.92474C32.0762 10.3217 27.3775 10.396 26.896 10.396V13.9661C27.3755 13.9661 32.0762 14.0404 32.2246 17.4374C32.0971 20.3909 29.1016 21.2721 27.9579 21.3425C27.9579 21.3425 25.0842 21.5918 23.1697 19.9894L20.5264 21.6698C23.3258 24.6043 27.7847 24.3569 27.7847 24.3569V24.3512C31.7241 24.2142 35.5512 21.9153 35.6577 17.4374C35.6007 15.0909 34.5197 13.3419 32.9382 12.1887L32.9363 12.1868Z"
                            fill="#C20A6C"
                        />
                        <path
                            d="M13.352 17.2355L10.4098 17.245L18.8043 12.1847H18.7986L18.8043 12.179L10.4098 7.1187L13.352 7.12821L22.0396 10.6679C21.2955 5.2632 16.7052 1.08594 11.0969 1.08594C4.96704 1.08594 0 6.05297 0 12.1809C0 18.3088 4.96704 23.2777 11.0969 23.2777C16.7052 23.2777 21.2955 19.1005 22.0396 13.6957L13.352 17.2355Z"
                            fill="#C20A6C"
                        />
                    </svg>
                    <div class="flex flex-col">
                        <span
                            class="text-[10px] font-jost-regular text-white leading-tight"
                        >
                            ОСНОВАНО 2020
                        </span>
                        <span
                            class="text-lg font-jost-bold text-white leading-tight"
                        >
                            ЗАТОЧКА<span class="text-[#C20A6C]">.</span>ТСК
                        </span>
                        <span
                            class="text-[10px] font-jost-regular text-white leading-tight"
                        >
                            ПОРА ЗАТОЧИТЬ ИНСТРУМЕНТЫ
                        </span>
                    </div>
                </a>

                <!-- Правая часть - кнопка входа и закрытия -->
                <div class="flex items-center space-x-3">
                    <!-- Кнопка входа -->
                    <a href="{{ url('/client/dashboard') }}"
                       
                        class="bg-[#C20A6C] hover:bg-[#a0085a] text-white px-4 py-2 rounded-xl font-jost-bold text-sm transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105 transform focus:outline-none focus:ring-2 focus:ring-white/50 whitespace-nowrap"
                    >
                        ВОЙТИ
                    </a>
                    <!-- Кнопка закрытия -->
                    <button type="button" data-mobile-menu-toggle
                        class="w-10 h-10 flex items-center justify-center text-white hover:text-[#C20A6C] transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-white/50 rounded-lg"
                        aria-label="Закрыть меню"
                    >
                        <svg
                            class="w-6 h-6"
                            fill="none"
                            stroke="currentColor"
                            stroke-width="2"
                            viewBox="0 0 24 24"
                        >
                            <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M6 18L18 6M6 6l12 12"
                            />
                        </svg>
                    </button>
                </div>
            </div>

            <!-- Средняя часть - Навигационные ссылки -->
            <nav
                class="flex-1 flex flex-col justify-left items-center space-y-6 px-6 mt-12"
            >
                <a href="{{ url('/') }}"
                    class="text-white text-2xl font-jost-bold transition-all duration-300 hover:scale-105{{ $site->isActive('/') ? ' bg-white/20' : '' }}"
                >
                    ГЛАВНАЯ
                </a>
                <a href="{{ url('/sharpening') }}"
                    class="text-white text-2xl font-jost-bold transition-all duration-300 hover:scale-105{{ $site->isActive('/sharpening') ? ' bg-white/20' : '' }}"
                >
                    ЗАТОЧКА
                </a>
                <a href="{{ url('/repair') }}"
                    class="text-white text-2xl font-jost-bold transition-all duration-300 hover:scale-105{{ $site->isActive('/repair') ? ' bg-white/20' : '' }}"
                >
                    РЕМОНТ
                </a>
                <a href="{{ url('/delivery') }}"
                    class="text-white text-2xl font-jost-bold transition-all duration-300 hover:scale-105{{ $site->isActive('/delivery') ? ' bg-white/20' : '' }}"
                >
                    ДОСТАВКА
                </a>
                <a href="{{ url('/prices') }}"
                    class="text-white text-2xl font-jost-bold transition-all duration-300 hover:scale-105{{ $site->isActive('/prices') ? ' bg-white/20' : '' }}"
                >
                    ПРАЙС
                </a>
                <a href="{{ url('/work-schedule') }}"
                    class="text-white text-2xl font-jost-bold transition-all duration-300 hover:scale-105{{ $site->isActive('/work-schedule') ? ' bg-white/20' : '' }}"
                >
                    ГРАФИК РАБОТЫ
                </a>
                <a href="{{ url('/contacts') }}"
                    class="text-white text-2xl font-jost-bold transition-all duration-300 hover:scale-105{{ $site->isActive('/contacts') ? ' bg-white/20' : '' }}"
                >
                    КОНТАКТЫ
                </a>
            </nav>

            <!-- Нижняя часть - Социальные иконки и инструменты -->
            <div class="relative pb-4 mt-auto">
                <div
                    class="absolute bottom-0 left-1/2 -translate-x-1/2 w-[120%] h-[120px] bg-white rounded-t-full"
                    style="border-radius: 50% 50% 0 0 / 100% 100% 0 0"
                ></div>

                <div class="relative px-6 z-10 pt-4 pb-2">
                    <!-- Иконки инструментов -->
                    <div class="flex justify-center items-center gap-4 mb-3">
                        <svg
                            width="98"
                            height="33"
                            viewBox="0 0 98 33"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            class="w-auto h-6"
                        >
                            <path
                                d="M75.7866 25.0391C76.1404 22.0891 81.5193 15.1516 84.1059 11.8191C84.8709 10.8342 85.4733 10.0548 85.7889 9.60062C85.8701 9.48109 85.9562 9.36156 86.0423 9.23725C86.1283 9.36156 86.2144 9.48587 86.2957 9.60062C86.6065 10.0548 87.2136 10.8342 87.9786 11.8191C90.5652 15.1516 95.9393 22.0891 96.2979 25.0391C96.776 29.0074 94.7679 31.661 94.7488 31.6897L96.0302 32.689C96.1306 32.5599 98.4733 29.5047 97.9139 24.843C97.5362 21.7114 93.2523 15.9644 89.2648 10.8246C88.5141 9.8588 87.926 9.09381 87.6392 8.67785C87.5531 8.55354 87.4623 8.42445 87.3715 8.29057C87.2567 8.12801 87.1371 7.95589 87.0224 7.78377C87.7874 6.56457 88.5285 5.11109 88.7437 3.49026C88.8823 2.45275 88.4855 1.68776 88.2512 1.00405L86.3531 0C86.7786 1.22398 87.1993 2.8209 87.1371 3.27511C86.9937 4.33653 86.5682 5.33102 86.0566 6.24423C85.5403 5.33102 85.1147 4.33653 84.9761 3.27511C84.9187 2.8209 85.3347 1.22398 85.7602 0L83.9577 1.01839C83.7425 1.64473 83.3457 2.07982 83.3457 3.20817C83.3457 3.30858 83.3505 3.4042 83.36 3.49026C83.5752 5.11109 84.3115 6.55979 85.0813 7.78377C84.9617 7.95589 84.8422 8.12801 84.7322 8.29057C84.6414 8.42445 84.5505 8.55354 84.4645 8.67785C84.1776 9.09381 83.5895 9.8588 82.8388 10.8246C78.8513 15.9644 74.5674 21.7114 74.1945 24.843C74.1323 25.3403 74.1084 25.8184 74.1084 26.2774C74.1084 30.1215 75.9875 32.5694 76.0783 32.6842L77.3596 31.6849C77.3405 31.6562 75.3324 29.0027 75.8105 25.0343L75.7866 25.0391Z"
                                fill="#C20A6C"
                            />
                            <path
                                d="M4.38435 30.4594C5.97648 30.4594 7.37259 29.6036 8.14236 28.3318C8.91213 29.6036 10.3082 30.4594 11.9004 30.4594C14.3149 30.4594 16.2847 28.4944 16.2847 26.0751C16.2847 23.804 14.5491 21.9298 12.3307 21.7147L8.82129 13.1659L10.9967 7.86356C11.231 7.32328 11.427 6.76867 11.5992 6.20927L10.5808 5.25781C10.3752 5.97499 10.1409 6.68261 9.84446 7.37588L8.14714 11.5116L6.44504 7.36631C6.1486 6.67782 5.91432 5.97021 5.70873 5.25781L4.69034 6.20927C4.86246 6.76389 5.05849 7.31372 5.28799 7.84921L7.46821 13.1611L3.95882 21.7099C1.73557 21.9346 0 23.8088 0 26.0799C0 28.4944 1.96507 30.4642 4.38435 30.4642V30.4594ZM15.0273 26.0751C15.0273 27.8011 13.6216 29.2068 11.8956 29.2068C10.1696 29.2068 8.76391 27.8011 8.76391 26.0751C8.76391 24.3491 10.1696 22.9434 11.8956 22.9434C13.6216 22.9434 15.0273 24.3491 15.0273 26.0751ZM8.14236 14.8154L11.0015 21.7864C9.79187 22.0398 8.76391 22.7952 8.14236 23.8232C7.52558 22.7952 6.49285 22.0398 5.28321 21.7864L8.14236 14.8154ZM4.38435 22.9434C6.11035 22.9434 7.51602 24.3491 7.51602 26.0751C7.51602 27.8011 6.11035 29.2068 4.38435 29.2068C2.65834 29.2068 1.25267 27.8011 1.25267 26.0751C1.25267 24.3491 2.65834 22.9434 4.38435 22.9434Z"
                                fill="#C20A6C"
                            />
                            <path
                                d="M60.8165 10.5076V13.6584H67.6393C67.9309 13.6584 68.1652 13.8926 68.1652 14.1843C68.1652 14.4759 67.9309 14.7102 67.6393 14.7102H60.8165V17.861H67.6393C67.9309 17.861 68.1652 18.0953 68.1652 18.3869C68.1652 18.6786 67.9309 18.9129 67.6393 18.9129H60.8165V22.0637H67.6393C67.9309 22.0637 68.1652 22.298 68.1652 22.5896C68.1652 22.8813 67.9309 23.1155 67.6393 23.1155H60.8165V29.9383C60.8165 30.2299 60.5822 30.4642 60.2906 30.4642C59.9989 30.4642 59.7646 30.2299 59.7646 29.9383V8.93454C59.7646 6.90732 61.4142 5.25781 63.4414 5.25781H67.6393C67.9309 5.25781 68.1652 5.49209 68.1652 5.78374C68.1652 6.0754 67.9309 6.30967 67.6393 6.30967H63.4414C61.9927 6.30967 60.8165 7.48584 60.8165 8.93454V9.46047H67.6393C67.9309 9.46047 68.1652 9.69475 68.1652 9.9864C68.1652 10.2781 67.9309 10.5123 67.6393 10.5123H60.8165V10.5076Z"
                                fill="#C20A6C"
                            />
                            <path
                                d="M24.7282 28.9486C24.4174 28.8482 24.0349 28.7047 23.6477 28.547C23.6477 28.131 23.662 27.7246 23.6907 27.3995C23.7194 27.0744 23.8533 26.754 24.0684 26.4958C26.3968 23.7132 34.1663 11.8654 39.7172 7.84443C41.1085 6.84517 46.4778 6.55351 48.4285 6.52961L46.1097 5.25781C43.7382 5.40603 40.2575 5.77896 38.9235 6.73998C32.6745 11.2391 23.2604 25.334 22.6532 26.0703L22.591 26.1468L22.5528 26.2329C22.2229 26.9883 22.2802 28.8816 22.3041 29.4458C22.8157 29.6849 24.5465 30.4594 25.3689 30.4499H25.4645L25.5554 30.4212C26.4686 30.1391 43.0449 26.7875 49.5999 22.7426C51.0008 21.882 52.6838 18.8077 53.7309 16.6753L53.4488 14.0456C52.6742 15.8386 50.3458 20.6867 48.8875 21.5856C43.0401 25.1714 29.1699 27.8728 25.7036 28.9534C25.3832 29.0538 25.0438 29.0538 24.7234 28.9534L24.7282 28.9486Z"
                                fill="#C20A6C"
                            />
                        </svg>
                    </div>
                    <!-- Социальные ссылки -->
                    @if(count($site->socialLinks()) > 0)
                    <div class="flex justify-center items-center gap-3 flex-wrap">
                        @foreach($site->socialLinks() as $link)
                            <a
                                href="{{ $link['url'] ?? '#' }}"
                                target="_blank"
                                rel="noopener"
                                class="px-3 py-2 bg-[#003859] text-white text-xs font-jost-bold rounded-full hover:scale-105 transition-transform duration-300"
                                aria-label="{{ $link['name'] ?? '' }}"
                            >
                                {{ $link['name'] ?? '' }}
                            </a>
                        @endforeach
                    </div>
                    @endif
                </div>
            </div>
        </div>
    </div>

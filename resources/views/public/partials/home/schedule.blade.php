        ...
        <div class="space-y-6">
            @foreach($site->scheduleDays() as $day)
                <div class="relative border border-dark-blue-500/30 dark:border-dark-gray-200/90 px-4 sm:px-6 pt-8 pb-4 bg-white/80 backdrop-blur-xl dark:bg-transparent">
                    <h3 class="absolute top-0 left-4 sm:left-6 -translate-y-1/2 px-3 bg-white dark:bg-dark-blue-500/90 text-lg sm:text-xl font-jost-bold uppercase tracking-wide {{ !empty($day['is_day_off']) ? 'text-[#C20A6C]' : 'text-dark-blue-500 dark:text-dark-blue-300' }}">
                        {{ $day['name'] ?? '' }}
                    </h3>
                    <div class="space-y-2 text-sm sm:text-base">
                        @if(!empty($day['is_day_off']))
                            <p class="font-jost-bold text-[#C20A6C]">{{ $day['day_off_text'] ?? 'Выходной' }}</p>
                        @else
                            <p class="font-jost-regular text-dark-gray-500 dark:text-gray-200">{{ $day['workshop'] ?? '' }}</p>
                            <p class="font-jost-regular text-[#C20A6C]">{{ $day['delivery'] ?? '' }}</p>
                        @endif
                    </div>
                </div>
            @endforeach

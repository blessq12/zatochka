...
<template>
    <div class="flex items-center gap-3 min-w-0">
        ...
        <div class="flex flex-col min-w-0 leading-tight">
            <span
                v-if="!compact"
                class="text-[10px] uppercase tracking-wider text-white/70 font-jost-medium"
            >
                Основано 2020
            </span>
            <span class="font-jost-bold text-base sm:text-lg truncate">
                Заточка<span class="text-white/90">.</span>ТСК
            </span>
            <span
                v-if="tagline"
                class="text-xs text-white/75 font-jost-medium truncate"
            >
                {{ tagline }}
            </span>
        </div>

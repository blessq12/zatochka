...
├── Filament/Resources/              # /cp
└── Models/User.php
```

| BC | Domain Entity | Presentation |
|----|---------------|----------------|
| OrderFulfillment | Order, OrderWork, OrderTool, OrderMaterial | Filament, POS `/api/pos/orders/*`, PDF |
| ClientPortal | Client, SiteLead, Review | `/api/leads`, `/api/client/*`, Vue `/client/dashboard`, Filament |
| Catalog | Branch, PriceBlock, PriceItem, SiteSetting | `GET /api/bootstrap` |
| Equipment | Equipment | Filament, POS read-only |
| Warehouse | WarehouseItem, StockMovement | Filament, POS read-only |
| Identity | Master | POS login, Filament web auth |

**Вертикаль POS** — не BC: API + Vue `/pos` поверх OrderFulfillment, Equipment, Warehouse, Identity. См. `.cursor/rules/POS/`.

**Реализовано:** все слои по 6 BC; Application handlers; API; Filament; POS SPA; domain events (синхронно через `event()`).

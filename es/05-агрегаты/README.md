...
```
app/Domain/
├── OrderFulfillment/     # Order, Work, Tool, OrderMaterial, Lead (convert)
├── ClientPortal/         # Client, Review, Lead (submit)
├── Equipment/
├── Warehouse/
├── Catalog/              # PriceBlock, Branch
└── Identity/             # или app/Models/User + policies
```

**Правило:** cross-BC — только `*_id` и application services; не тянуть Eloquent-связи через границы BC в доменной логике.
